...
def load_segmentation_files(basepath, train_test):
    ...
    if ['train', 'val'].__contains__(train_test):
        segmentation_path = basepath + '/segmentations/final_segmentations/train_val_balanced'
    elif train_test == 'test':
        segmentation_path = basepath + '/segmentations/final_segmentations/test_balanced'
    else:
...
def prepare_and_write_masked_data_bern(basepath,
    ...
    # ==========================================
    if ['train', 'val'].__contains__(train_test):
        seg_path = basepath + f'/segmentations/final_segmentations/train_val_balanced'
        ...
        seg_path = basepath + f'/segmentations/final_segmentations/test_balanced'
...
def prepare_and_write_sliced_data_bern(basepath,
    ...
    hand_seg_path_controls = basepath + '/segmentations/segmenter_rw_pw_hard/controls'
    hand_seg_path_patients = basepath + '/segmentations/segmenter_rw_pw_hard/patients'
    list_hand_seg_images = os.listdir(hand_seg_path_controls) + os.listdir(hand_seg_path_patients)
    # Sort the list
    list_hand_seg_images.sort()
    if ['train', 'val'].__contains__(train_test):

        seg_path = basepath + '/segmentations/final_segmentations/train_val_balanced'
        img_path = basepath + '/preprocessed/controls/numpy'
    elif train_test == 'test':
        # For the img_path we need to look into the patients folder or the controls folder, try both, see further down
        seg_path = basepath + '/segmentations/final_segmentations/test_balanced'
...
def prepare_and_write_masked_data_sliced_bern(basepath,
    ...
    hand_seg_path_controls = [
        os.path.join(basepath, 'segmentations/segmenter_rw_pw_hard', 'controls'),
        os.path.join(basepath, 'segmentations/segmenter_rw_pw_hard', 'controls_compressed_sensing')
    ]
    hand_seg_path_patients = [
        os.path.join(basepath, 'segmentations/segmenter_rw_pw_hard', 'patients'),
        os.path.join(basepath, 'segmentations/segmenter_rw_pw_hard', 'patients_compressed_sensing')
    ]
    ...
    if train_test in ['train', 'val'] and not only_compressed_sensing:
        ...
        seg_path = basepath + '/segmentations/final_segmentations/train_val_compressed_sensing_balanced'
        
    elif train_test == 'test' and only_compressed_sensing:
        seg_path = basepath + f'/segmentations/final_segmentations/test_compressed_sensing_balanced'
    ...
    # Add a check that the number of patients corresponds to the expected number (idx_end - idx_start)
    expected_num_patients = idx_end - idx_start
    if num_images_to_load != expected_num_patients:
        raise ValueError(f"ERROR: Number of patients ({num_images_to_load}) does not match the expected number ({expected_num_patients}). Total number of patients: {len(filtered_seg_path_files)}")
...
# Set up argument parsing
    ...
    make_dir_safely(os.path.join(sys_config.project_code_root, 'logs', 'preprocess_logs'))
    ...
    masked_sliced_data_train_all = load_masked_data_sliced(basepath, idx_start=0, idx_end=1, train_test='train', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])
    masked_sliced_data_validation_all = load_masked_data_sliced(basepath, idx_start=0, idx_end=1, train_test='val', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])
    masked_sliced_data_test_all = load_masked_data_sliced(basepath, idx_start=0, idx_end=1, train_test='test', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])

    masked_sliced_data_train_all = load_masked_data_sliced(basepath, idx_start=config['idx_start_train'], idx_end=config['idx_end_train'], train_test='train', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])
    masked_sliced_data_validation_all = load_masked_data_sliced(basepath, idx_start=config['idx_start_val'], idx_end=config['idx_end_val'], train_test='val', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])
    masked_sliced_data_test_all = load_masked_data_sliced(basepath, idx_start=config['idx_start_test'], idx_end=config['idx_end_test'], train_test='test', suffix = suffix, include_compressed_sensing = config['include_compressed_sensing_data'], only_compressed_sensing = config['use_only_compressed_sensing_data'], skip = config['skip_points_on_centerline'], updated_ao = config['use_updated_ordering_method'], smoothness = config['smoothing'], force_overwrite= config['force_overwrite'])