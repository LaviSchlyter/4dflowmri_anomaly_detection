...
from models.condconv import CondVAE
...
def plot_scores(healthy_scores,sick_scores, level, agg_function = np.mean, data = "test", deformation = None, note = None):
    ...
    if level == 'patient':
        ...
        df_sick = pd.DataFrame({
            'Patient': ['Sick '+str(i) for i in range(len(sick_means))],
            'Mean Score': sick_means,
            'Std Deviation': sick_stds,
            'Status': ['Sick']*len(sick_means)
        })

        # Create a dataframe for healthy patients
        df_healthy = pd.DataFrame({
            'Patient': ['Healthy '+str(i) for i in range(len(healthy_means))],
            'Mean Score': healthy_means,
            'Std Deviation': healthy_stds,
            'Status': ['Healthy']*len(healthy_means)
        })
        ...
        sns.barplot(x='Patient', y='Mean Score', hue='Status', data=df, yerr=df['Std Deviation'], capsize=.2)
        plt.xlabel('Patient')
        ...
        if agg_function == np.mean:
            plt.title('Mean Anomaly Score with Standard Deviation for Each Patient')
            ...
            plt.title('Sum Anomaly Score with Standard Deviation for Each Patient')
            plt.ylabel('Sum Anomaly Score')
            if note:
                plt.savefig(os.path.join(save_dir+ '/' + f'{data}_patient_sum_wise_scores_{note}.png'))
                plt.close()
            else:
                plt.savefig(os.path.join(save_dir+ '/' + f'{data}_patient_sum_wise_scores.png'))
                plt.close()
        ...
        sick_df = pd.DataFrame({
            'Patient Status': np.repeat('Sick', sick_means.size),
            'Patient': np.repeat(np.arange(sick_means.shape[0]), sick_means.shape[1]),
        ...
        healthy_df = pd.DataFrame({
            'Patient Status': np.repeat('Healthy', healthy_means.size),
            'Patient': np.repeat(np.arange(healthy_means.shape[0]), healthy_means.shape[1]),
        ...
        plt.figure(figsize=(12, 6))
        line_plot = sns.lineplot(data=df, x='imagewise', y='Mean Score', hue='Patient Status', style="Patient")
        line_plot.legend_.remove()  # Remove the original legend
        plt.xlabel('z_slice')
        # Create a new legend only for Patient Status (Color)
        handles, labels = line_plot.get_legend_handles_labels()
        line_plot.legend(handles=handles[:3], labels=labels[:3], title='Patient Status', loc='upper left')

        if agg_function == np.mean:

            plt.title('Mean Anomaly Score with Standard Deviation for Each Patient Group and imagewise')
            plt.ylabel('Mean Anomaly Score')
            if note:        
            ...
            plt.title('Sum Anomaly Score with Standard Deviation for Each Patient Group and imagewise')
            plt.ylabel('Sum Anomaly Score')
            if note:
                plt.savefig(os.path.join(save_dir+ '/' + f'{data}_sum_imagewise_scores_{note}.png'))
            else:
                plt.savefig(os.path.join(save_dir+ '/' + f'{data}_sum_imagewise_scores.png'))
        plt.close()
        # Create the line plot
        plt.figure(figsize=(12, 6))
        sns.lineplot(data=df, x='imagewise', y='Mean Score', hue='Patient Status', err_style="bars", errorbar='sd')
        plt.xlabel('z_slice')
        ...
        if note:
            ...
            sick_min_index = df.loc[df['Patient Status'] == 'Sick']['Mean Score'].idxmin()
            sick_max_index = df.loc[df['Patient Status'] == 'Sick']['Mean Score'].idxmax()
            healthy_min_index = df.loc[df['Patient Status'] == 'Healthy']['Mean Score'].idxmin()
            healthy_max_index = df.loc[df['Patient Status'] == 'Healthy']['Mean Score'].idxmax()
            # Get the imagewise indexes
            sick_min_index = df.loc[df['Patient Status'] == 'Sick']['imagewise'][sick_min_index]
            sick_max_index = df.loc[df['Patient Status'] == 'Sick']['imagewise'][sick_max_index]
            healthy_min_index = df.loc[df['Patient Status'] == 'Healthy']['imagewise'][healthy_min_index]
            healthy_max_index = df.loc[df['Patient Status'] == 'Healthy']['imagewise'][healthy_max_index]
...
if __name__ == '__main__':
    ...
    for model_rel_path in list_of_experiments_paths:
        ...
        if "test" in data_to_visualize:
            ...
            logging.info('============================================================')
            logging.info('Healthy subjects anomaly_score: {} +/- {:.4e}'.format(healthy_mean_anomaly_score, healthy_std_anomaly_score))
